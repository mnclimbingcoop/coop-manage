...
<%@ page import="coop.mnclimbing.User" %>
<html>
  <head>
	...
	<g:form method="post" >
	  ...
	  <div class="dialog">
		<table>
		  ...
		  <g:link controller="person" action="edit" id="${userInstance?.person?.id}" >${userInstance?.person}
		  </g:link>
		  <g:hiddenField name="person.id" value="${userInstance?.person?.id}" />
		  </td>
		  </tr>
		  ...
		  <tr class="prop">
			...
			<td valign="top" class="value ${hasErrors(bean: userInstance, field: 'passwordExpired', 'errors')}">
			  <ul>
			  <g:each var="r" in="${coop.mnclimbing.Role.list()}">
				<li>
				  <g:checkBox name="authority_${r.id}" value="${r.id}" checked="${userInstance.authorities.find{ it.id == r.id} }" />
				  ${r.authority}
				</li>
			  </g:each>
			  </ul>
